  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-200/50">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 h-11 sm:h-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {currentUser?.role === "CUSTOMER" && <button onClick={() => nav("/cart")} className="relative p-2 rounded-lg hover:bg-gray-100"><ShoppingCart className="w-4.5 h-4.5 text-gray-700" />{cart.length > 0 && <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-green-600 text-white text-[10px] rounded-full flex items-center justify-center font-bold">{cart.reduce((s, i) => s + i.quantity, 0)}</span>}</button>}
